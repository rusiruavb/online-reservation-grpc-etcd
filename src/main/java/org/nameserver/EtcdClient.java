...
public class EtcdClient {
    ...
    public String get(String key) throws IOException { System.out.println("Getting value for Key=" +
            key);
    ...
    private String buildPutRequestPayload(String key, String value) {
        String keyEncoded =
                Base64.getEncoder().encodeToString(key.getBytes()); String valueEncoded =
                Base64.getEncoder().encodeToString(value.getBytes()); JSONObject putRequest = new JSONObject(); putRequest.put("key", keyEncoded); putRequest.put("value", valueEncoded);
        return putRequest.toString();
    }
    private String buildGetRequestPayload(String key) {
        String keyEncoded =
                Base64.getEncoder().encodeToString(key.getBytes()); JSONObject putRequest = new JSONObject(); putRequest.put("key", keyEncoded);
        return putRequest.toString();
    }
}
